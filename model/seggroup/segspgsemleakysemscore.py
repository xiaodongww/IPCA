...
@author: Xiaodong Wu
@version: 1.0
@file: segspgsemleakysemscore.py

Full model of our method including IPCA module and SCN module.
...
class SegSPGSemLeakySemScore(nn.Module):
    def __init__(self, cfg):
        ...
        self.obj_unet = UBlock([m, 2 * m], norm_fn, 2, block, indice_key_id=1)
        self.obj_outputlayer = spconv.SparseSequential(
            norm_fn(m),
            nn.LeakyReLU(0.01)
        )
        self.obj_linear = nn.Linear(m, cfg.classes)
        ...
        module_map = {'input_conv': self.input_conv, 'unet': self.unet, 'output_layer': self.output_layer,
                      ...
                      'score_linear': self.score_linear, 'segment_layer': self.segment_layer, 'obj_unet': self.obj_unet,
                      'obj_outputlayer': self.obj_outputlayer,
                      'obj_linear': self.obj_linear}
        ...
        module_map['segsem_layer'] = self.segsem_layer
    ...
    def forward(self, input, input_map, coords, batch_idxs, batch_offsets, spg_edgs, spg_edg_offsets, seg_pcd_idxs,
                uni_seg_labels, spg_edg_idxs, epoch, other_input={}, mode='train'):
        '''
        ...
        if (epoch > self.prepare_epochs):
            ...
            if mode == 'train':
                instance_labels = other_input['instance_labels']
                semantic_labels = other_input['semantic_labels']
                # instance_labels[semantic_labels < 0] = -100
                obj_idx, obj_offsets = instlabel2idx(instance_labels)
                obj_labels = semantic_labels[obj_idx[:, 1][obj_offsets[:-1].long()].long()].to(semantic_scores.device)

                obj_idx = obj_idx.type(torch.int32)
                obj_offsets = obj_offsets.type(torch.int32)
                obj_input_feats, obj_map = self.clusters_voxelization(obj_idx, obj_offsets, output_feats, coords,
                                                                      self.score_fullscale, self.score_scale, self.mode)

                obj_score = self.obj_unet(obj_input_feats)
                obj_score = self.obj_outputlayer(obj_score)
                obj_score_feats = obj_score.features[obj_map.long()]  # (sumNPoint, C)
                obj_score_feats = pointgroup_ops.roipool(obj_score_feats, obj_offsets.cuda())  # (nProposal, C)
                obj_scores = self.obj_linear(obj_score_feats)  # (nProposal, 1)

                ret['obj_scores'] = (obj_scores, obj_labels)
            else:
                obj_score = self.obj_unet(input_feats)
                obj_score = self.obj_outputlayer(obj_score)
                obj_score_feats = obj_score.features[inp_map.long()]  # (sumNPoint, C)
                obj_score_feats = pointgroup_ops.roipool(obj_score_feats, proposals_offset.cuda())  # (nProposal, C)
                obj_scores = self.obj_linear(obj_score_feats)  # (nProposal, 1)
                ret['obj_scores'] = obj_scores

        return ret


def instlabel2idx(instance_labels):
    """

    :param instance_labels: torch.Tensor, N
    :return:
    """
    inst_locs = []
    inst_labels = []
    inst_offsets = [0]
    for instid in sorted(torch.unique(instance_labels)):
        if instid < 0:
            continue
        inst_loc = torch.where(instance_labels == instid)[0]
        inst_locs.append(inst_loc)
        inst_labels += [instid] * len(inst_loc)
        inst_offsets.append(len(inst_labels))
    inst_locs = torch.cat(inst_locs)
    inst_labels = torch.tensor(inst_labels)
    inst_offsets = torch.tensor(inst_offsets).to(instance_labels.device)

    inst_idx = torch.stack([inst_labels, inst_locs]).T.contiguous().to(instance_labels.device)
    return inst_idx, inst_offsets
...
def model_fn_decorator(cfg, test=False):
    ...
    def test_model_fn(batch, model, epoch):
        ...
        ret = model(input_, p2v_map, coords_float, coords[:, 0].int(), batch_offsets, spg_edgs, spg_edg_offsets,
                    ...
                    other_input=other_input, mode='test')
        ...
        with torch.no_grad():
            preds = {}
            preds['seg_sem_scores'] = seg_sem_scores
            ...
            if (epoch > cfg.prepare_epochs):
                ...
                preds['obj_scores'] = ret['obj_scores']
    ...
    def model_fn(batch, model, epoch):
        ...
        other_input['over_seg_label'] = batch['normal_seg_labels']
        other_input['seg_offsets'] = batch['seg_offsets']
        other_input['instance_labels'] = batch['instance_labels']
        other_input['semantic_labels'] = batch['labels']
        if cfg.use_overseg:
        ...
        if (epoch > cfg.prepare_epochs):
            ...
            loss_inp['obj_scores'] = ret['obj_scores']
    ...
    def loss_fn(loss_inp, epoch):
        ...
        if (epoch > cfg.prepare_epochs):
            ...
            obj_scores, obj_labels = loss_inp['obj_scores']
            obj_loss = semantic_criterion(obj_scores, obj_labels.cuda())

            loss_out['object_loss'] = (obj_loss, obj_scores.shape[0])
