    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(dataset.test_data_loader):
            ...
            if (epoch > cfg.prepare_epochs):
                ...
                ### use predicted iou as proposal score
                ...
                #----- use most common class as proposal class -----#
            ...
            if (epoch > cfg.prepare_epochs and cfg.save_instance):
                ...
                for proposal_id in range(nclusters):
                    ...
                    semantic_label = np.argmax(np.bincount(semantic_pred[np.where(clusters_i == 1)[0]].cpu()))  # TODO debug  如果是基于bbox的实例分割则该行要修改
